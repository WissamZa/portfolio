...
export default function DashboardQuickLinks({ locale }: { locale: string }) {
    const quickLinks = ADMIN_NAV_ITEMS
        .filter(item => !['dashboard', 'audit-logs', 'settings'].includes(item.key))
        .map(item => ({
            ...item,
